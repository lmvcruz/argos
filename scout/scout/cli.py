...
def get_github_credentials(args) -> Tuple[str, str, Optional[str]]:
    """
    Get GitHub credentials from arguments, environment, or .scoutrc file.

    Checks in this order:
    1. Command-line arguments (--token, --repo)
    2. Environment variables (GITHUB_TOKEN, GITHUB_REPO)
    3. .scoutrc config file in ~/.scout/<owner>/<repo>/
    ...
    # Initialize repo data manager for config access
    rdm = RepoDataManager(owner=owner, repo=repo_name)
    rdm.initialize()

    # Get token with fallback to .scoutrc
    token = args.token or os.environ.get("GITHUB_TOKEN")
    if not token:
        # Try loading from .scoutrc
        token = rdm.get_config_value("token")

    if not token:
        raise ValueError(
            "GitHub token required. Provide via --token option, "
            "GITHUB_TOKEN environment variable, or store in "
            f"~/.scout/{owner}/{repo_name}/.scoutrc"
        )

    return owner, repo_name, token


def get_repo_data_manager(args) -> RepoDataManager:
    """
    Get a RepoDataManager instance for the given arguments.

    Args:
        args: Parsed command-line arguments (must have --repo)

    Returns:
        RepoDataManager instance initialized for the repo

    Raises:
        ValueError: If repository is missing or invalid
    """
    owner, repo_name, _ = get_github_credentials(args)
    rdm = RepoDataManager(owner=owner, repo=repo_name)
    rdm.initialize()
    return rdm
...
def get_db_path(args, rdm: Optional[RepoDataManager] = None) -> str:
    """
    Get the database path for the given arguments.

    If args.db is provided, use it; otherwise, use the repo-specific path.

    Args:
        args: Parsed command-line arguments
        rdm: Optional RepoDataManager instance (will be created if not provided)
...
def get_ci_github_client(args):
    ...
    # Get repo data manager for paths
    rdm = RepoDataManager(owner=owner, repo=repo_name)
    rdm.initialize()

    # Use repo-specific database path or custom path if provided
    db_path = args.db if args.db else str(rdm.get_db_path())
