...
class SyncOutputFormatter:
    ...
    def parse_completed(test_count: int, passed: int, failed: int, skipped: int):
        ...
        print(
            f"    {SyncOutputFormatter.GREEN}✓{SyncOutputFormatter.RESET} Parsed successfully")
        print(
    ...
    def parse_skipped(reason: str = "already in database"):
        ...
        print(
            f"    {SyncOutputFormatter.YELLOW}⊘{SyncOutputFormatter.RESET} Skipped ({reason})")

    @staticmethod
    def error(message: str):
        """Print error message."""
        print(
            f"{SyncOutputFormatter.RED}[✗] Error{SyncOutputFormatter.RESET}: {message}")
    ...
    def summary(
        ...
        print("\n" + "=" * 70)
        print(
            f"{SyncOutputFormatter.BOLD}Scout CI Sync Summary{SyncOutputFormatter.RESET}")
        print("=" * 70)
        ...
        if errors:
            ...
            for error in errors:
                print(
                    f"  {SyncOutputFormatter.RED}✗{SyncOutputFormatter.RESET} {error}")
