...
class SyncOutputFormatter:
    ...
    @staticmethod
    def header(workflow: Optional[str], limit: Optional[int], force_download: bool, force_parse: bool):
        """Print operation header."""
        print("\n" + "=" * 70)
        print(f"{SyncOutputFormatter.BOLD}Scout CI Sync - Started at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}{SyncOutputFormatter.RESET}")
        print("=" * 70)
    ...
    def download_started(workflow_name: str, run_number: int):
        ...
        print(f"{SyncOutputFormatter.BLUE}[↓] Downloading{SyncOutputFormatter.RESET} {workflow_name} #{run_number}")
    
    @staticmethod
    def download_completed(workflow_name: str, job_name: str, lines: int, size_kb: float):
        """Print download completed message."""
        print(f"    {SyncOutputFormatter.GREEN}✓{SyncOutputFormatter.RESET} Downloaded job '{job_name}'")
    ...
    def download_skipped(job_name: str, reason: str = "already cached"):
        ...
        print(f"    {SyncOutputFormatter.YELLOW}⊘{SyncOutputFormatter.RESET} Skipped job '{job_name}' ({reason})")
    ...
    def parse_started(workflow_name: str, job_name: str, parser_names: List[str]):
        ...
        print(f"{SyncOutputFormatter.BLUE}[⟲] Parsing{SyncOutputFormatter.RESET} {workflow_name}/{job_name}")
    ...
    def parse_completed(test_count: int, passed: int, failed: int, skipped: int):
        ...
        print(f"      Results: {test_count} tests ({passed} passed, {failed} failed, {skipped} skipped)")
